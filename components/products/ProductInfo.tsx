...
import {CartButton, FavoriteButton} from "../ui";
...
export const ProductInfo: FC<Props> = ({product}) => {
  ...
  return (
    <div className="flex items-start justify-center px-5 pb-5 md:pb-0 md:mt-0 md:items-center">
      <div className="flex flex-col justify-center gap-4 md:gap-10 md:w-3/4 md:h-3/4 text-primary ">
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mt-2 md:mt-4">
            <button className="relative inline-flex items-center justify-start w-5 h-5 overflow-hidden font-medium transition-all bg-white rounded-full hover:bg-primary group">
        ...
        <div>
          ...
          <form className="flex gap-4 mt-4">
            {product.sizes.map((s) => (
              ...
              >
                <span
                  className={`w-48 h-48 rotate-[-40deg] bg-[#50949C] absolute bottom-0 left-0 -translate-x-full 
